...
$(function () {
  ...
  $('#copyreport').click(function () {
    ...
    if (hour < 2 && date > 1)
      date--;
    var report = `#отчёт_чаты ${date} ${monthNames[month]} ${$('#shiftfrom').children("option:selected").val()}-${$('#shiftto').children("option:selected").val()}`;
    ...
    report += '\n\n**💙 Хорошей смены:** хорошей смены, котики :)'
    try {
  ...
  socket.on('dataTask', data => {
    switch (data) {
      ...
      default:
        ...
        if (typeof data.incident.chronology !== 'undefined')
  ...
  $('#copyincident').click(function () {
    ...
    if ($('#addToReport').prop('checked')) {
      var index = structure.report.field.name.indexOf('🚨 Инциденты и ДТП');
      var idText = `#${structure.report.field.id[index]}Text`;
  ...
  $('#saveToken').click(function () {
    setCookie('token', $('#tokenField').val(), 'max-age=2678400')
  })
});
