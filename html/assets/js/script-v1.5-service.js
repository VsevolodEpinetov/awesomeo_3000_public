...
$(function () {
  ...
  structure.report.field.name = [
    "🔧 Текущие задачи по сервисам", 
    "🦸‍♂️ Текущие задачи старшим техникам", 
    "✍️ Топ-5 запросов", 
    "🛠 Жалобы на сервисное приложение"
  ]
  structure.report.field.placeholder = [
    "🔧 Текущие задачи по сервисам", 
    "🦸‍♂️ Текущиек задачи старшим техникам", 
    "✍️ Топ-5 запросов", 
    "🛠 Жалобы на сервисное приложение"
  ]
  ...
  $('#copyreport').click(function () {
    ...
    if (hour < 2 && date > 1)
      date--;
    var report = `#отчет_сервисы ${date} ${monthNames[month]} ${$('#shiftfrom').children("option:selected").val()}-${$('#shiftto').children("option:selected").val()}`;
